...
const computeReturnDate = ({ returnType, returnDays, returnMonths, returnDate }) => {
  const now = new Date()
  if (returnType === 'days' && returnDays) {
    const d = new Date(now)
    d.setDate(d.getDate() + parseInt(returnDays))
    return d
  }
  if (returnType === 'months' && returnMonths) {
    const d = new Date(now)
    d.setMonth(d.getMonth() + parseInt(returnMonths))
    return d
  }
  if (returnType === 'date' && returnDate) {
...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    if (requestType === 'device' && !deviceId) return res.status(400).json({ error: 'deviceId required for device assignment' })
    if (requestType === 'set' && !setId) return res.status(400).json({ error: 'setId required for set assignment' })
    if (!['ok', 'repair', 'damage'].includes(healthStatus)) return res.status(400).json({ error: 'Invalid healthStatus' })
    if ((healthStatus === 'repair' || healthStatus === 'damage') && !healthComment?.trim()) return res.status(400).json({ error: 'healthComment is required when healthStatus is repair or damage' })
    ...
    const request = await prisma.assignmentRequest.create({
      data: {
        ...
        healthComment: healthComment?.trim() || null,
        returnType,
        returnDays: returnDays ? parseInt(returnDays) : null,
        returnMonths: returnMonths ? parseInt(returnMonths) : null,
...
// On approval: atomically move device/set to 'assigning' + link clientId
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (request.requestType === 'device' && request.deviceId) {
        await tx.device.update({
          ...
          data: { clientId: request.clientId, lifecycleStatus: 'assigning', healthStatus: request.healthStatus },
        })
        ...
        await tx.device.updateMany({
          ...
          data: { clientId: request.clientId, lifecycleStatus: 'assigning' },
        })
      }
...
router.get('/set/:setId', authMiddleware, async (req, res) => {
  try {
